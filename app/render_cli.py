...
from app.config import (
    ...
    MANIM_DISABLE_CACHING,
    MANIM_OUTPUT_DIR,
    MANIM_PROGRESS_BAR,
    MANIM_QUALITY,
    MANIM_RENDER_FAILED_MARKER,
    MANIM_RENDER_OK_MARKER,
    MANIM_RENDERER,
    MANIM_SPEC_SIDECAR_EXTENSION,
    MANIM_TEMP_DIR_PREFIX,
    MANIM_VERBOSITY,
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        print(
            f"{MANIM_RENDER_FAILED_MARKER}usage: python -m app.render_cli <spec.json>",
            file=sys.stderr,
            flush=True,
        )
        return 2
