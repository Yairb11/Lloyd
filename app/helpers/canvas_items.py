...
from app.helpers.canvas_geometry import qp
...
def center_text(item: QGraphicsTextItem, x: float, y: float, view_scale: float) -> None:
    rect = item.boundingRect()
    item.setPos(qp(x, y) - QPointF(rect.width() / 2 / view_scale, rect.height() / 2 / view_scale))
...
def make_vessel(shape, label: str) -> ShapeItem:
    return ShapeItem(
        vessel_path(shape),
        fill=None,
        stroke="#FFFFFF",
